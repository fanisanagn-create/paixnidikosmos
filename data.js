/* Content data + Supabase integration */
...
const CATEGORY_CARDS = [
  { id: "toys", title: "Παιχνίδια", count: "320+ προιόντα", art: "blocks", theme: "yellow", desc: "Κλασικά, νέα, αγαπημένα" },
  { id: "edu", title: "Εκπαιδευτικά", count: "180+ προιόντα", art: "book", theme: "cream", desc: "Παιχνίδι που μαθαίνει" },
...
const PRODUCTS = [
  ...
  { id: 11, name: "Κουτί Δώρου ΍κπληξη", cat: "gifts", price: 34.90, tag: "ΕΚΠΛΗΞΗ", art: "gift", bg: "red", age: "Όλες" },
...
const OFFERS = [
  { badge: "Καλοκαιρινή προσφορά", title: "΍κπτωση 30%", titleAccent: "σε όλα τα εξωτερικά παιχνίδια", text: "Φουσκωτές πισίνες, ποδήλατα, παιχνίδια θαλάσσης — όλα -30% μέχρι τέλος Ιουνίου.", art: "rocket" },
  { badge: "Πακέτο γενεθλίων", title: "Δωρεάν", titleAccent: "συσκευασία δώρου", text: "Σε κάθε αγορά πάνω από 25€. Διαλέγεις χρώμα κορδέλας και κάρτα — το ετοιμάζουμε εμείς.", art: "gift" },
  { badge: "Πιστοί πελάτες", title: "1 + 1 = 3", titleAccent: "στις κηρομπογιές", text: "Φέρε δύο πακέτα κηρομπογιές, παίρνεις και τρίτο δωρεάν. Όλο τον μήνα.", art: "crayons" },
];
...
async function loadProductsFromSheets() {
  ...
  try {
    ...
    window.PRODUCTS = data.map(r => ({
      id: r.id, name: r.name || '', cat: r.cat || 'toys',
      price: parseFloat(r.price) || 0, oldPrice: r.old_price ? parseFloat(r.old_price) : null,
      tag: r.tag || null, art: r.art || 'gift', bg: r.bg || 'yellow', age: r.age || 'Όλες', image: r.image || null,
    }));
  } catch (e) { console.warn('[Supabase] Αδυναμία φόρτωσης προιόντων', e.message); }
}
...
async function loadOffersFromSheets() {
  ...
  if (!sb) return;
  try {
    const { data, error } = await sb.from('offers').select('*').eq('active', true).order('sort_order', { ascending: true });
    if (error) throw error;
    if (!data || data.length === 0) return;
    window.OFFERS = data.map(r => ({
      badge: r.badge || '', title: r.title || '', titleAccent: r.title_accent || '', text: r.text || '', art: r.art || 'gift',
    }));
  } catch (e) { console.warn('[Supabase] Αδυναμία φόρτωσης προσφορών', e.message); }
}
...
window.getSetting = (key, fallback) =>
  (window.SITE_SETTINGS && window.SITE_SETTINGS[key] !== undefined && window.SITE_SETTINGS[key] !== '')
    ? window.SITE_SETTINGS[key] : (fallback !== undefined ? fallback : '');
...
async function loadSettingsFromSupabase() {
  ...
  } catch (e) { console.warn('[Supabase] Αδυναμία φόρτωσης ρυθμίσεων', e.message); }
}
